...
Case of 
	: (($ev=On Mouse Move:K2:35) | ($ev=On Mouse Enter:K2:33))
		...
		If (Form:C1466.cacheInfoPersons.length#0)
			
			GET MOUSE:C468($x;$y;$z)
			
			$result:=cellCoordinates ($x-640;$y-530)
			
			If ($result.row<=Form:C1466.cacheInfoPersons.length)
				$entry:=Form:C1466.cacheInfoPersons[$result.row-1]["col"+String:C10($result.column-1)]
